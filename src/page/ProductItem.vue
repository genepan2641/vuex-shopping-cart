		...
		<template v-else>
			<div class="item-page-container">
				...
				<div class="item-page__right">
					<div class="item-info">
						<p class="item-info__name">{{ productDetail.name }}</p>
						<p>
							<span class="item-info__price">NT${{ productDetail.price }}</span>
						</p>
						...
						<div class="mB20">
							...
							<select v-model="selectedSize" class="form-control" name id>
								...
								<option
									v-for="(sizeAmount, sizeName) in productDetail.sizes"
									:value="sizeName"
									:key="sizeName"
								>{{ sizeName }}</option>
							</select>
						...
						<p v-if="selectedSize">
							<template v-if="availableAmount">size: {{ selectedSize }} {{ availableAmount }} left</template>
							<template v-else>size: {{ selectedSize }} is not available now</template>
						</p>
